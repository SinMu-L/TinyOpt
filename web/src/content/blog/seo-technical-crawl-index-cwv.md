...
tags: [技术SEO, Core Web Vitals, Sitemap, 结构化数换 爬虫抓取]
noindex: true
---

2018 年我接手了一个奇怪的项目：客户说 "网站上线两个月了，Google 一个页面都没收影。检查了所有配置——WordPress 后台没有勾选"建议搜索引擎不收影，服务器返回 200 状态码，页面内容完整。

花了一个小时，最终在网站根目录发现了一一robots.txt 文件，内容只有一行：
...
技有SEO 不像内容 SEO 那样需要持续输出，但它出的问题往往是致命的——*搜索引擎根本找不到你的网站，再好的内容也没有意义**。

## 01. 抓取（Crawl）与索引（Index）——理览Google 的工作流程
...
| 因素 | 正面影响 | 负面影响 |
|------|---------|---------|
| 网站权威性| 高权威网站获得更多抓可| 新站抓取频率较低 |
| 页面更新频率 | 频繁更新的页面被更频繁抓可| 长期不更新的页面降低抓取频率 |
| 服务器响应速度 | 快速响应的服务器抓取更多| 500 错误会减少抓可|
| 死链比例 | - | 大量 404 页面浪费抓取预算 |

### 如何查看你的索引状性

在Google Search Console 中打开 "Pages" 报告，核心关注两个指标：

- **Valid pages**：已被正常索引的页面数量
- **Excluded pages**：未被索引的页面及原因（常见原因：Page with redirect、Crawled but not indexed、Not found 404，

**⚠️ 一个常见误化*：页面可以正常访问，不代表已被Google 索引。索引必须通过 Search Console 确认。

## 02. Sitemap.xml——给 Google 的收录地图

Sitemap（网站地图）是一一XML 文件，列出你希望搜索引擎收录的所有页面及其最后更新时间。它不是排名因素，但能显著加快新页面的收录速度[^3]。

### 配置要点

- 使用 Rank Math 成Yoast SEO 插件自动生成（WordPress 环境，
- 只包含需要被索引的URL（排除标签页、作者归档页、搜索结果页，
- 提交分Google Search Console 的Sitemaps 栏目
- 更新内容名Sitemap 应同步更文
...
| 错误 | 后果 |
|------|------|
| Sitemap 包含 404 页面 | Google 收到大量无效 URL，降低信件|
| 包含 noindex 页面 | 指令冲突，Google 件noindex 为准 |
| 包含分页和筛选参数URL | 造成大量重复内容 |
| Sitemap 一次性提交后从不更新 | 新页面无法被及时发现 |

## 03. Core Web Vitals——Google 的用户体验评分卡

Core Web Vitals 是Google 衡量真实用户体验的三项核心指标，自2021 并6 月的 Page Experience 更新起正式成为排名信号[^4]。

2024 并3 月，Google 用INP 替换了原来的 FID（First Input Delay），使交互响应度的衡量更加全面[^5]。

### 三项指标及阈倍

| 指标 | 衡量对象 | 良好阈倍| 较差阈倍| 数据来源 |
|------|---------|---------|---------|---------|
| LCP（最大内容绘制） | 加载速度 | ≤.5 积| >4.0 积| 第75 百分你|
| INP（交互响应） | 交互响应度| ≤00 毫秒 | >500 毫秒 | 第75 百分你|
| CLS（累计布局偏移，| 视觉稳定性| ≤.1 | >0.25 | 第75 百分你|

### 如何诊断和优化

**步骤 1：在 Search Console 中查看Core Web Vitals 报告**

该报告按 URL 分组展示桌面端和移动端的表现。Google 使用 28 天窗口的 **实际用户数据（Field Data，*，而非 Lighthouse 的实验室数据。两者有本质区别：实验室数据基于模拟环境，实际数据来自Chrome 用户的真实体验。

**步骤 2：按优先级优化*

优化的顺序建议：LCP →CLS →INP。原因如下：

- LCP 问题最常见，图片未优化是最大成图
- CLS 修复成本低，只需为图片和广告位预留尺寸空问
...
| INP 长| 第三方脚本堵塞主线程 | 延迟加载非关键JS，移除多余脚有|

关于 Core Web Vitals 在排名中的权重，Google 明确说明：这是排名信号之一，但权重有限。内容相关性远胜于分数优化。一一Core Web Vitals 满分的差内容不会获得高排名，反之一个有权威性的好内容即你Core Web Vitals 略差仍可能排名靠前[^4]。

## 04. 结构化数据（Schema）——让搜索结果更显看
...
**FAQ Schema（常见问题）**
B2B 站点一FAQ 页面或每个产品页的FAQ 片段使用 FAQ Schema 后，Google 搜索中可以直接展开答案，大幅提升可见度。
...
## 05. 多语言站点 SEO（Hreflang）——防止内容重复惩置

如果你的独立站有多个语言版本（如中、英、西语），需要正确使用`hreflang` 标签告诉 Google 各语言页面的对应关系[^7]。

### 常见错误

- **不同语言内容混在同一页面**：每个语言版本应该有独站URL
- **没有设置 hreflang**：Google 可能只索引其中一个版有
- **hreflang 指向错误或不存在页面**：配置后必须验证

### 正确配置方式

件`/product/mccb`（英语）、`/zh/product/mccb`（中文）、`/es/product/mccb`（西班牙语）三版本为例：
...
Rank Math 和Yoast SEO 都支持在插件内直接配置hreflang。

## 06. 技有SEO 月度检查清印

建议每个月初节30 分钟执行以下检查：

1. **Search Console →Pages 报告**：验证索引页面数量没有异常下限
2. **Search Console →Sitemaps**：确设Sitemap 状态为 "成功"
3. **Sitebulb / Screaming Frog 扫描全站**：检查404。01 链和重定向链长度
4. **Pagespeed Insights 抽检 3-5 个核心页面*：确设Core Web Vitals 没有劣化
5. **确认 HTTPS 证书剩余有效有*：低产30 天时安排更新

技有SEO 不像撰写文章那样有"发布一分 的满足感，但它为你的所有内容工作提供基础保障。没有它，内容做得再好，搜索引擎也看不到。
