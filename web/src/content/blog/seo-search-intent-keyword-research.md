...
月初帮一个做工业阀门的客户做 SEO 诊断。他的独立站上线 8 个月，发布了 60 多篇产品文章，Google Search Console 里收录了 200 多个页面，但带来源自然流量的关键词只有 11 个。问题出在哪？他告诉我："我每篇文章都围绕核心产品词写的，比如 industrial valve。"

这正是最常见的 SEO 误区——**把搜索量当唯一指标，忽略搜索意图**。
...
|---------|------------|----------|--------------|
| 信息型 Informational | 获取知识 | "what is mccb" / "how does a valve work" | 博客文章、指南、FAQ |
| 商业调查型 Commercial | 比较方案 | "best mccb manufacturer" / "valve vs gate valve" | 对比文章、评测、买家指南 |
| 交易型 Transactional | 完成购买 | "buy 630a mccb" / "valve price list" | 产品页、落地页、购物车 |
| 导航型 Navigational | 找特定网站 | "siemens mccb" / "facebook login" | 品牌首页、登录页 |

去年我们为一个做电气设备的 B2B 独立站做关键词重组。他们之前的策略是所有页面都围绕 "circuit breaker" 优化，结果 15 个页面在同一个词上互相竞争，没有一个进前三。重新按意图分类后：
...
三个月后，主产品词从第 11 位升至第 3 位，整站自然流量增长 170%。

## 02. 关键词研究的四步实战流程

基于我们服务过 30 多个 B2B 独立站的经验，这是经过验证的流程：

**第一步：建立关键词种子库**

从以下五个维度收集原始关键词，每个维度至少列出 10-20 个词：

- 产品词：产品名称、型号、规格（如 mccb, 630a mccb）
- 应用词：使用场景、行业（如 industrial electrical protection, power distribution）
- 问题词：客户常问的问题（如 how to select mccb, mccb vs mcb difference）
- 采购词：购买意图词（如 mccb supplier, mccb factory, mccb wholesale）
- 长尾词：带修饰的精准词（如 3-pole 630a mccb for industrial use）
...
- Google 搜索下拉框 → 输入种子词，记录自动补全
- "People Also Ask" 区块 → 用户真实关心的问题

付费工具层面，Semrush 的 Keyword Magic Tool 和 Ahrefs 的 Keywords Explorer 能提供搜索量、关键词难度（KD）、CPC 等核心数据[^3]。成本敏感时可以先用 Google 的免费工具组合。
...
这是大多数 B2B 站点最容易出错的环节。核心原则是：**一个主要搜索意图对应一个主要页面**。
...
## 03. 关键词内耗（Keyword Cannibalization）——最容易忽视的陷阱
...
在 Google 搜索 `site:yourdomain.com "核心关键词"`，如果出现 3 个以上你的页面，说明存在内耗风险。更精确的方法是：在 Search Console 中筛选某个关键词，查看哪些页面获得了展示。

### 修复策略

| 场景 | 解决方案 |
|------|---------|
| 两篇文章内容相似，意图相同 | 合并为一篇，另一篇 301 重定向 |
| 意图不同但关键词有重叠 | 无需合并，确保两篇有明显区分 |
| 老文章内容过时 | 更新老文章，取消新文章的索引 |

根据 Moz 的研究，合并内耗页面后，74% 的案例在 4-8 周内看到了排名回升[^7]。

## 04. EEAT 视角：关键词研究也是专业度的体现

Google 在 2022 年 12 月将 E-A-T 更新为 E-E-A-T，新增的 "Experience"（经验）维度强调内容创作者的真实使用经验[^8]。
...
这类长尾搜索词背后是真实的使用场景和经验诉求，不仅竞争低，而且对应的内容天然带有 EEAT 优势——只有真正做过维护的人才能写出有价值的内容。
...
1. 列出你网站的主要产品词和服务词，每种至少 10 个
2. 用 Search Console 或 Google 搜索下拉框验证这些词的实际搜索表现
3. 检查是否有多个页面在争夺同一个关键词——如果有，制定合并或区分计划

做 B2B 独立站的人最大的幻觉是："先把关键词抄进来，后面再优化。"事实上，第一步的方向决定了后面 90% 的 SEO 效果。
