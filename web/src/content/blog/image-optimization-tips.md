...
description: 5 个经过验证的图片压缩技巧：选对文件格式类型、控制展示尺寸大小、调优压缩参数、用批量处理替代逐张手动操作、把重命名与格式转换合并到压缩流程。附 WebP/AVIF/JPEG 实测数据对比，把体积压到最小画质却看不出区别。
tags: [使用技巧, 图片优化, 性能优化]
---
...
👉 [TinyOpt 支持 9 格式互转，下载试试](/download/)

## 02. 不要上传比显示尺寸更大的图

4000px 宽的照片上传到页面，显示宽度只有 800px——每次加载都在浪费 5 倍的带宽。

**做法**：压缩前先缩放到实际显示尺寸。TinyOpt 的适配模式（Fit）可以按目标宽度等比缩放，一步到位。
...
TinyOpt 支持批量拖入，一次性处理数百张图片。配合多 Key 自动轮询，几百张图也就是一个拖拽 + 一次点击的工作量。

## 05. 把重命名和格式转换合并到压缩流程里

很多人压缩完图片才发现还要手动改名、还要转格式——白白多做一遍。

TinyOpt 允许在同一个任务中同时完成：压缩 + 格式转换 + 批量重命名。一次配置，全部搞定。
...
立即开始：[下载 TinyOpt 批量压缩助手](/download/)
