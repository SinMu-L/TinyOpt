---
title: 产品图加载时"加购"按钮飞了？CLS 布局偏移修复指南（电商版）
date: 2026-07-17
lang: zh
translationKey: cls-layout-shift-fix
description: 产品图没设宽高、Review 区域动态渲染是电商站 CLS 布局偏移的常见原因。本文拆解 5 个产品页 CLS 罪魁祸首和 3 步修复流程，帮你把 CLS 降到 0.1 以内，避免 Core Web Vitals 扣分伤害排名。
tags: [CLS, Shopify优化, 布局偏移, 产品页性能, PageSpeed]
---

你的 Shopify 店铺，产品图压缩了，WebP 也换了，CDN 也上了。打开 PageSpeed，分数还是黄的。点开 CLS 一看：0.38（不通过）。

问题不在图片大小，在布局偏移。更多细节在加载时，页面不断跳动——先看到加购按钮，手指刚要按，图加载完了，按钮被挤到页面下方。

这才是让人崩溃的部分：你已经做了所有"据说有效"的优化，但指标就是不动。你修的是体积，而 CLS 扣分的是**页面跳动**。
