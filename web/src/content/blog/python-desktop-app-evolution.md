---
title: Python 桌面应用进化史：�?200 行脚本到 3000 行产�?
date: 2026-06-16
lang: zh
translationKey: python-desktop-app-evolution
description: 一次真实的重构经历复盘：一个图片压缩小工具如何在半年内�?200 行命令行脚本演化为包含水印、批量重命名、多 Key 并发管理�?3000 �?PyQt5 桌面级应用，以及我在架构持续迭代演进中总结�?5 条核心经验教训�?
tags: [Python, PyQt5, 架构设计, 桌面应用, 重构]
noindex: true
---

半年前我写了一�?Python 脚本：调�?TinyPNG API，压缩图片，完事。大�?200 行，够用�?

然后用户开始提需求—�?能不能加水印�?"能不能批量重命名�?"能同时用多个 API Key 吗？"

每个需求单独看都不大。但当它们堆在一起，那个 200 行的脚本开始变得难以维护。一个功能改了，另一个功能崩了。全局变量到处飞，没有模块边界，没有状态管理�?

于是我开始重构。半年后，它长成了一�?3000 行的 PyQt5 桌面应用�? 个独立功能模块，双语国际化，自动构建和发布�?
...
**最大问�?*：每次要压缩不同目录的图片都得改脚本�?

这在只有我自己用时没问题。当我把工具分享给朋友后，问题来了——朋友不会改 Python 脚本�?

## 02. 第二阶段：加 GUI，但架构没跟�?

为了让朋友也能用，我加了 PyQt5 GUI。这一阶段犯了一个典型错误：**直接把逻辑�?UI 塞在一�?*�?

压缩按钮的点击回调里，写着完整�?API 调用逻辑�?
...
一个函数做�?5 件事。加个进度条都要小心翼翼�?

更糟的是，PyQt �?GUI 线程不能阻塞。我被迫引入 QThread，但没有明确的设计——每个新功能都自己开线程，状态同步全靠信号，�?bug 时根本不知道是哪�?Worker 出了问题�?

| 问题 | 表现 |
|------|------|
| 职责混杂 | UI 里夹�?API 调用、文件处理、错误上�?|
| 线程失控 | 每个功能各自�?QThread，无统一管理 |
| 状态混�?| API Key �?3 个不同地方被读取 |
...
重构的核心思路�?**按职责分模块，用 Worker 模式解耦线�?*�?

### 3.1 统一 Worker 模式

每个耗时操作都封装为独立�?QThread Worker�?

| Worker | 职责 | 信号 |
|--------|------|------|
| `CompressWorker` | 调用 TinyPNG API，处理压�?转换/缩放 | `progress`, `file_done`, `all_done` |
| `WatermarkWorker` | 读取图片、合成水印、保存输�?| `progress`, `file_done`, `all_done` |
...
API Key 从各个散落的地方抽出来，统一�?`KeyManager` 管理�?
...
有了这个抽象层，并发压缩变得简单：Worker �?KeyManager �?Key，用完归还，KeyManager 自动做轮询、配额检查和失效禁用�?

### 3.3 i18n 系统

加上双语支持时，我不想给每个窗口写两�?UI 代码。于是做了一个简单的 Translator 类：
...
窗口在初始化时注册回调，语言切换后所�?label、button、table header 自动刷新，不需要重启�?

## 04. 第四阶段：CI/CD 与自动发�?

模块化之后，发布成了一个新瓶颈——每次改完代码要手动�?PyInstaller、上传到网盘、通知用户�?
...
**�?全局变量是万恶之�?*
API Key �?3 个地方读取、状态在 5 个地方修改——这种耦合让改一个功能变得不可能。用单一管理类收拢状态，代价最小，收益最大�?

**�?i18n 要尽早做**
我的应用是中文界面。当用户�?有没有英文版"时，我花�?3 天把所有硬编码的字符串从代码里抽出来。如果在第一天就�?JSON 存字符串，只需�?3 小时�?
...
| �?| 组件 | 职责 |
|----|------|------|
| UI | MainWindow + 5 Tab | 用户交互、文件拖拽、配置管�?|
| Worker | Compress / Watermark / Rename | 后台任务、进度上�?|
| 核心 | KeyManager / Translator | API Key 调度、国际化 |
| 数据 | JSON 配置文件 | 持久化配置和压缩历史 |
| 构建 | PyInstaller + GitHub Actions | 打包、发布、分�?|
