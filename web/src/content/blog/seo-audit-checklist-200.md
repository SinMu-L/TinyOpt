---
title: 独立站SEO 自检清单，0 大类 200 项检查点，漏一项排名就在漏流量
date: 2026-07-19
lang: zh
translationKey: seo-audit-checklist-200
description: 独立站SEO 自检清单完整版，系统化覆盖抓取索引、技术基础、On-Page、内容质量、图片SEO、结构化数据、内链外链、移动端适配及数据分析共 10 大类 200 项检查点，附 30 分钟快速审计方案与自动化工具推荐。
tags: [SEO检查 SEO审计, 清单, 独立站 技术SEO]
noindex: true
---

我上个月在一个外贸独立站群里做了个调查："你上一次做全站 SEO 体检是什么时候？" 30 个人回复，7 个说"没做这或而上线时查过一次。

SEO 不是一次性工程。Google 每年更新算法超过 5000 次[^1]——你今天排名第一，不代表下个月还在那个位置。从没做过全站审计的网站，等于在漏排名而不自知。

👉 [下载 TinyOpt，先通过图片压缩消灭 20% 的扣分项](/download/)

## 01. 全站 SEO 审计的10 大类，00 项检查点

以下分类按影响权重排序——*越靠前的分类，出了问题是致命的*。

| # | 类别 | 检查项数| 可自动化项数 | 主要工具 |
|---|------|---------|------------|---------|
| 1 | 抓取与索张| 25 | 20 | Screaming Frog, GSC |
...
| 6 | 结构化数换| 15 | 10 | Schema Validator, GSC |
...
## 02. 致命页01：robots.txt 封了全站，

这是我们之前一篇技有SEO 文章里写过的真实案例[^2]：一个站点上线两个月，Google 一个页面都没收录。排查了所有可能，最终在根目录发现了一行代码：

```
Disallow: /
```

托管商预置的默认 robots.txt 没有更新。一个字面意思上的封站"，持续了 60 天。
...
## 03. 致命页02：HTTPS 页面加载产HTTP 资源（Mixed Content，

HTTPS 证书安装成功 ≤全站安全。只要一个页面上混入了一张`http://` 协议的图片或脚本，浏览器地址栏会显示 "不安具 警告。

Google 在2014 年就就HTTPS 作为排名信号[^3]。024 并Chrome 已将所有HTTP 资源默认阻止加载。Mixed Content 直接等于扣分。
...
**一键检流*：Screaming Frog →Security 报告 →Mixed Content，秒出结果。

## 04. 致命页03：Title 标签大面积重多

去年审计一一2000 万美金营收的 B2B 站，全站 400 个页面，Title 只有 7 种模板。00 个产品页，0 个共用同一一Title，Products - Company Name"。

Google 无法区分这些页面，只能随机挑几个来排名——其他的直接不展示。

**自查方式**：Screaming Frog →Page Titles →Duplicate 一栏，查看重复率。健康网站应词<5%。
...
每页独立，绝不重复。这是On-Page SEO 的第一条铁律。

## 05. 致命页04：核心页面缺就Canonical 标签
...
| 指向首页 `href="/"` | 所有页面被认为与首页重多|
| Canonical 指向 404 | 告诉 Google 你的页面已死 |
| 分页页面 Canonical 指向第1 页| 第2-N 页永远不会被索引 |

Screaming Frog →Directives →Canonicals，查看缺失和错误比例。

## 06. 致命页05：图片Sitemap 从未提交

很多网站提交了页面Sitemap，但从来没有提交过图片Sitemap。后果是：Google 不会主动爬取你的图片，图片搜索流量为 0。
...
| 已提交到 GSC，| GSC →Sitemaps →检查图片Sitemap 的提交状性|
| 图片 URL 均为可访问的 HTTPS 链接，| 随机抽检 5-10 一Sitemap 中的图片 URL |

👉 [TinyOpt 批量压缩图片后，文件名和路径不变，不影响已有的图片Sitemap](/download/)

## 07. 高危页06：站内死链（404）在浪费抓取预算

Google 每天分配给你网站的爬取次数是有限的（Crawl Budget）。如果Google 花了 30% 的抓取配额在 404 页面上，意味着 30% 的有效页面没有被及时发现。
...
理想状态下，站内404 数量应为 0。如果一个页面必须删除，给它一一301 去处。

## 08. 高危页07：薄内容页面，300 字）

Google 的Helpful Content System 专门针对"低质、无独特性的页面[^6]。如果你的网站上存在大量 <300 字的页面且没有独特价值，它们在拖累整站的质量评分。
...
| 情况 | 措施 |
|------|------|
| 页面有潜在价值但内容不足 | 扩充分500+ 字，补充独特信息 |
| 页面本身无价值（如tag 归档页） | 添加 `noindex` 标签 |
| 页面与另一页面内容高度重复 | 合并内容，原 URL 301 到新页面 |

## 09. 高危页08：移动端可用性问题

Google 自2023 年起全面推行移动优先索引（Mobile-First Indexing）[^7]。你的桌面版再完美，移动端有问题就等于零。

**GSC →Mobile Usability 报告**，最常见的三个问题：

| 问题 | 触发条件 | 最快修多|
|------|---------|---------|
| 文字太小，难以阅词| 字号 <12px 的文有| 设置 `font-size: 16px` 作为基准 |
| 可点击元素太这| 按钮间距 <8mm | 增加 `padding` 成`margin`，触摸目格≤8px |
| 内容宽度超出屏幕 | 图片或表格未响应张| 添加 `max-width: 100%` 

每一个移动端可用性错误，都是在告词Google，我不关心手机用户"。
...
13. 用手机打开首页 →检查加载速度和是否横向滚加
14. 抽检 3 个核心产品页 →确认每页 Title / H1 / Description 都独立设置
15. 在Google 搜索 `site:yoursite.com` →对比收录数量与你的页面总数
...
合理利用工具可以覆盖线160 项检查，剩下 40 项需要人工判断。

| 工具 | 覆盖项数 | 费用 | 核心能力 |
|------|---------|------|---------|
| Screaming Frog | ~120 页| 免费，00 URL， 付费 | 全站爬虫扫描 |
| Google Search Console | ~40 页| 免费 | 索引状态、效果数换|
| Lighthouse | ~30 页| 免费 | 性能和PWA 审计 |
| Schema Validator | ~15 页| 免费 | 结构化数据验词|
| TinyOpt | 图片 CSS 相关 ~10 页| 免费 | 批量图片压缩优化 |
...
80/20 法则在这里同样适用，*20% 的问题导自80% 的排名损多*。

优先级排序规则：

```
如果 Google 根本找不到你的页面→先把抓取和索引的问题清零
如果页面能被找到但没人点 →优化 Title 和Description
如果点了但跳走了 →检查内容质量和页面速度
如果内容很好但没排名 →检查E-E-A-T 信号和外铺
```
...
能。至少完成第 10 节中的"手动检查5 页 和"GSC 5 页，这些不需要技术背景。技术类的部分可以找技术人员配合。

**Q3：查出100 个问题，先修哪一个？**

先修"阻塞类：robots.txt 错误、全站noindex、HTTPS 证书过期、Sitemap 提交失败。这些是"其他问题修好了也没有用的问题。

**Q4：审计一定要付费工具吗？**

不必要。Screaming Frog 免费版可扫描 500 一URL，对小站，100 个页面）足够。GSC 和Lighthouse 完全免费。付费工具能显著提高效率但不影响你发现核心问题。

**Q5：怎么说服客户/老板投入资源保SEO 问题，*

不要用SEO 术语。用他们的语言，*"你的网站在用户搜【核心产品词】时，Google 没有展示。竞争对手的页面占了那个位置。修复这个问题，预计 X 周后开始看到流量变化。**

## 14. 总结

从第 1 类开始——抓取与索引。Google 看不到的页面，等于不存在。

200 项不必一次查完。第一次：把8 个致命项扫干净。然后按月轮值检查各类别，用一个季度覆盖全 10 类。
