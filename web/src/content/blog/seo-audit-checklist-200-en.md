...
SEO isn't a one-time thing. Google updates its algorithm over 5,000 times per year[^1] â€?ranking #1 today doesn't mean you'll hold that spot next month. A site that has never been audited is leaking rankings without knowing it.

ðŸ‘‰ [Download TinyOpt and eliminate 20% of SEO penalties through image compression first](/download/)

## 01. The 10 Categories and 200 Checkpoints

Categories are ordered by impact weight â€?**the earlier the category, the more fatal its problems**.
...
## 03. Critical #02: Mixed Content â€?HTTPS Page Loading HTTP Resources

HTTPS certificate installed â‰?site is fully secure. If a single image or script on any page loads over `http://`, the browser shows a "Not Secure" warning.
...
| JS/CSS | Browser blocks loading outright â€?page breaks |
| Embedded video/fonts | Content won't render |

**One-click detection**: Screaming Frog â†?Security â†?Mixed Content report. Results in seconds.

## 04. Critical #03: Title Tags Duplicated Across Pages

Last year I audited a B2B site with $20M annual revenue. Four hundred pages. Seven Title templates. Fifty product pages shared one identical Title: "Products â€?Company Name."

Google can't differentiate those pages. It randomly picks a few to rank and ignores the rest.

**How to check**: Screaming Frog â†?Page Titles â†?Duplicate column. A healthy site should be under 5%.
...
Canonical tags tell Google: "This is the official version of this page." Without them, Google might choose the wrong version to index â€?or think you're publishing duplicate content.
...
| Error | Consequence |
|-------|------------|
| All pages missing canonicals | Google decides which version to index â€?may choose wrong |
...
| Paginated pages canonical â†?page 1 | Pages 2-N will never be indexed |

Screaming Frog â†?Directives â†?Canonicals. Check the missing and error ratios.
...
| Submitted to GSC? | GSC â†?Sitemaps â†?check image sitemap submission status |
| All image URLs HTTPS and accessible? | Spot-check 5-10 image URLs from the sitemap |

ðŸ‘‰ [TinyOpt compresses images in bulk without changing filenames or paths â€?your existing image sitemap stays valid](/download/)
...
**How to check**: Screaming Frog â†?Response Codes â†?Client Error (4XX).
...
**GSC â†?Mobile Usability report**. Three most common issues:

| Issue | Trigger | Quickest Fix |
|-------|---------|-------------|
| Text too small to read | Font size <12px | Set `font-size: 16px` as baseline |
| Clickable elements too close | Button spacing <8mm | Increase padding/margin, touch targets â‰?8px |
...
1. Pages report â†?check "Not indexed" count and reasons
2. Sitemaps â†?confirm submission status is "Success"
3. Core Web Vitals â†?check mobile and desktop LCP/INP/CLS groupings
4. Mobile Usability â†?confirm zero errors
5. Security Issues â†?confirm no issues

**Screaming Frog quick scan (10 minutes, 5 items)**:

6. Response Codes â†?check 404 and 500 error counts
7. Page Titles â†?duplicate Title percentage
8. Meta Description â†?missing or duplicate percentage
9. Canonicals â†?missing percentage
10. H1 â†?pages with missing or duplicate H1

**Manual check (10 minutes, 5 items)**:

11. `yoursite.com/robots.txt` â†?confirm no `Disallow: /`
12. Type your domain in browser â†?confirm HTTPS auto-redirect, no security warning
13. Open homepage on phone â†?check load speed and horizontal scrolling
14. Spot-check 3 core product pages â†?confirm each has unique Title / H1 / Description
15. Google search `site:yoursite.com` â†?compare indexed count to your total page count

**Lighthouse (5 minutes, 5 items)**:

16. Performance score â‰?0 (mobile)
17. Accessibility score â‰?5
18. Best Practices score â‰?0
19. SEO score â‰?0
20. Check Opportunities â†?what's the single biggest improvement?
...
```
If Google can't find your pages â†?clear all crawl and index issues first
If pages are found but not clicked â†?optimize Title and Description
If clicked but users bounce â†?check content quality and page speed
If content is good but won't rank â†?check E-E-A-T signals and backlinks
```
...
Yes. At minimum, complete the "Manual check 5 items" and "GSC 5 items" from Section 10 â€?no technical background needed. Technical sections can be handed off to a developer.
...
No. Screaming Frog's free version scans 500 URLs â€?enough for small sites (<100 pages). GSC and Lighthouse are entirely free. Paid tools improve efficiency but don't change your ability to find core problems.
...
Start with Category 1 â€?Crawl & Index. A page Google can't see doesn't exist.
...
An SEO audit isn't a test with a passing score. It's a map â€?showing you where rankings are leaking and which hole to plug first.
